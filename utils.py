...
def evaluate_logits(model, loader, device, mask_class=None, eval=True, silent=True):
    """Evaluate a model trained with standard CE on a dataset."""
    model.to(device)
    model.eval()
    correct = 0
    total = 0
    for batch in tqdm(loader, 'Evaluating model', disable=silent):
        batch.to(device)
        with torch.no_grad():
            outputs = model(**batch)
...
def evaluate_cliphead(
        ...
        return_confusion=False, task_info=None, return_loss=False, silent=False, return_idxs=False):
    ...
    with torch.no_grad(), autocast():
        for inputs, labels in tqdm(loader, 'Evaluating CLIP head model', disable=silent, ncols=100):
    ...
    pred_result = torch.tensor(labels_list) == torch.tensor(preds_list)
    id_correct = torch.nonzero(pred_result == True).squeeze().tolist()
    idx_wrong = torch.nonzero(pred_result == False).squeeze().tolist()
    ...
    if return_confusion:
        ...
        if return_idxs:
            return correct / total, id_correct, idx_wrong
        return correct / total
...
def prepare_llama(config, device):
    ...
    ptm_model = AutoModelForSequenceClassification.from_pretrained(
        model_name_or_path, return_dict=True, cache_dir=config['cachedir'], num_labels=3).half()
    ...
    for base_path in tqdm(config['bases'], desc="Preparing Models"):
        if base_path.endswith('.pt'):
            ...
            adapter_model = PeftModel.from_pretrained(model=ptm_model, model_id=base_path).half()
    ...
    if model_name_or_path == 'meta-llama/Meta-Llama-3-8B':
        ...
        base_model = PeftModel.from_pretrained(model=ptm_model, model_id=ptm_model_path).half()
    else:
...
def parse_eval_args():
    ...
    parser.add_argument('--retain_rank', type=int, help='Retain rank')
    parser.add_argument('--prune_ratio', type=float, help='Pruning ratio')
    parser.add_argument('--prune_method', type=str, help='Pruning method')
    parser.add_argument('--drop_layer', type=int, help='Drop layer')
    parser.add_argument('--tasks', type=str, help='Tasks to perform')
    parser.add_argument('--task_alphas', type=str, help='Task scaling coefficients')
    parser.add_argument('--prune_indices', type=str, help='Task scaling coefficients')
