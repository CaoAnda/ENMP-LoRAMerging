...
class MatrixPerLayerMerger(TaskMerger):
    ...
    def get_core_matrices(self, A_list, B_list, key, merge_config, merge_cache):
        # --- disk cache ---
        cache_dir = ".cache"
        os.makedirs(cache_dir, exist_ok=True)

        # cache key based on merge_config + key
        h = hashlib.sha1()
        for t in A_list + B_list:
            # move to cpu/contiguous to avoid device-dependent hash
            h.update(str(t.sum().item()).encode())
        h.update(str(f"{merge_config}_{key}").encode())
        cache_key = h.hexdigest()

        cache_path = os.path.join(cache_dir, f"{cache_key}.pt")

        if os.path.exists(cache_path) and merge_cache:
            data = torch.load(cache_path, map_location=self.device)
            M_list, U_B_ref, Vh_A_ref = data["M_list"], data["U_B_ref"], data["Vh_A_ref"]
        ...
        Vh_A_ref = torch.linalg.svd(A_stack.to(torch.float64), full_matrices=False)[2]
        U_B_ref = torch.linalg.svd(B_stack.to(torch.float64), full_matrices=False)[0]

        M_list = []
        for i, (A, B) in enumerate(zip(A_list, B_list)):
            U_A, S_A, Vh_A = torch.linalg.svd(A.to(torch.float64), full_matrices=False)
            U_B, S_B, Vh_B = torch.linalg.svd(B.to(torch.float64), full_matrices=False)

            Q_A = Vh_A @ Vh_A_ref.T
            R_B = U_B_ref.T @ U_B

            # intermediate core matrix M = Σ_B * V_B^T * U_A * Σ_A
        ...
        # --- save cache ---
        if merge_cache:
            torch.save({"M_list": M_list, "U_B_ref": U_B_ref, "Vh_A_ref": Vh_A_ref}, cache_path)
    ...
    def get_knots_components(self, ftms_task_dirs):
        ...
        U, S, Vh = torch.linalg.svd(stack.to(torch.float32), full_matrices=False)
        # Keep only supported basis components
        U = U[:, S > 1e-5].type(torch.float32)
    ...
    def _merge_tensors(self, tensors, merge_config, merge_cache):
        cache_dir = ".cache"
        os.makedirs(cache_dir, exist_ok=True)
        ...
        for t in tensors:
            # move to cpu/contiguous to avoid device-dependent hash
            h.update(str(t.sum().item()).encode())
        h.update(str(merge_config).encode())
        ...
        if merge_config.get('merge_method') == 'mean':
            ...
            result = self._process_ties(tensors, merge_config.get('topK', 10))
            ...
            if self.lmc:
                ...
                result = torch.stack(tensors_dare).sum(dim=0)
            else:
                result = self.get_dare_delta_w(tensors, merge_config.get('dare_pruning_coeffs', 0.3), merge=True)

        elif merge_config.get('merge_method') == 'dare-ties':
            tensors_dare = self.get_dare_delta_w(tensors, merge_config.get('dare_pruning_coeffs', 0.3), merge=False)
            result = self._process_ties(tensors_dare, merge_config.get('topK', 10))
        elif merge_config.get('merge_method') == 'tsv':
            result = self.get_tsv_delta_w(tensors)
            result = result.type_as(tensors[0]) if hasattr(result, 'type_as') else result
        elif merge_config.get('merge_method') == 'cart':
            result = self.get_cart_delta_w(
                tensors,
        ...
        # atomic write: use a unique temp file then rename to avoid partial reads
        if merge_cache:
            temp_path = os.path.join(cache_dir, f"{cache_key}.tmp.{uuid.uuid4()}")
            try:
    ...
    def merge(self, merge_config, drop_state=None, merge_cache=False):

        # Determine merge space and prepare parameters
        if merge_config.get('merge_space') in ('core', 'separate_a_b', 'core-vector'):
            ftms_params_ab = [ftm.get_ft_ab_parameters(self.device) for ftm in self.ftms_params]
            relevant_ab_keys = self.ftms_params[0].get_ft_ab_parameters().keys()
        else:
            ftms_task_dirs = [ftm.get_ft_parameters(self.device) for ftm in self.ftms_params]
        ...
        if merge_config.get('merge_space') == 'full':
            for key in all_keys:
                ...
                if key_base in ftms_task_dirs[0]:
                    ...
                    if drop_state is not None:
                        layer_id = int(re.search(r'layers\.(\d+)\.', key_base).group(1))
                        zero = torch.zeros_like(tensor_list[0])
                        for model_id in range(len(tensor_list)-1, -1, -1):
                            if drop_state[layer_id, model_id]:
                                tensor_list.pop(model_id)
                        if len(tensor_list) == 0:
                            tensor_list.append(zero)
                            
                    delta_w = self._merge_tensors(tensor_list, merge_config, merge_cache)
                    if merge_config.get('isotropize', False):
            ...
            for key in tqdm(all_keys, desc="Merging knots space"):
                ...
                if key_base in ftms_task_dirs[0]:
                    ...
                    Vs_merged = self._merge_tensors(Vs, merge_config, merge_cache)
            ...
            for key in all_keys:
                ...
                if key_base in relevant_ab_keys:
                    ...
                    zero_A = torch.zeros_like(A_list[0])
                    zero_B = torch.zeros_like(B_list[0])
                    for model_id in range(len(ftms_params_ab)-1, -1, -1):
                        if drop_state[layer_id, model_id]:
                            A_list.pop(model_id)
                            B_list.pop(model_id)
                    if len(A_list) == 0:
                        A_list.append(zero_A)
                        B_list.append(zero_B)
                                
                    M_list, U_B_ref, Vh_A_ref = self.get_core_matrices(A_list, B_list, key_base, merge_config, merge_cache)
                                
                    M_merged = self._merge_tensors(M_list, merge_config, merge_cache)
...
def stable_hash(obj):
    """Recursively hash arbitrary objects (tensor/dict/list) to a stable string."""
    if isinstance(obj, torch.Tensor):
        t = obj.detach().cpu().contiguous()
        data = t.numpy().tobytes()
        meta = f"{t.shape}-{t.dtype}"
        return hashlib.sha1(meta.encode() + data).hexdigest()
    elif isinstance(obj, dict):
        # sort keys for stable ordering
        items = [(k, stable_hash(v)) for k, v in sorted(obj.items())]
        payload = json.dumps(items, sort_keys=True).encode()
        return hashlib.sha1(payload).hexdigest()
    elif isinstance(obj, (list, tuple)):
        payload = json.dumps([stable_hash(v) for v in obj]).encode()
        return hashlib.sha1(payload).hexdigest()
