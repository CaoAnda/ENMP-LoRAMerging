...
class LoRAHandler():
    ...
    def get_ft_parameters(self, device='cuda'):
        ...
        for name, key2val in layer2lora_parameters.items():
            ...
            task_parameters[name] = (key2val['B'].to(device) @ key2val['A'].to(device))
        return OrderedDict(sorted(task_parameters.items()))

    def get_ft_ab_parameters(self, device=None):
        layer2lora_parameters = defaultdict(lambda: dict())
        sd = self.state_dict
        for key, val in sd.items():
            if '.lora_A.default' in key:
        ...
        task_parameters = {}
        for name, key2val in layer2lora_parameters.items():
            task_parameters[name] = (key2val['A'].to(device), key2val['B'].to(device))
        return OrderedDict(sorted(task_parameters.items()))
