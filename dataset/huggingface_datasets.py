...
class HuggingFaceDataset:
    def __init__(self,
        ...
        # print(f"task is {task}, cache dir is {ROOT}, task id is {task_ids[task]}")
        ...
        if "train" in self.tokenized_datasets:
            self.train_loader = DataLoader(self.tokenized_datasets["train"], shuffle=True, collate_fn=self.collate_fn, batch_size=batch_size, num_workers=num_workers, pin_memory=True)
        if "test" in self.tokenized_datasets:
            self.test_loader = DataLoader(self.tokenized_datasets["test"], shuffle=False, collate_fn=self.collate_fn, batch_size=batch_size, num_workers=num_workers, pin_memory=True)
        if "validation" in self.tokenized_datasets:
            val_dataset = self.tokenized_datasets["validation"]
            if val_limit is not None and isinstance(val_limit, int):
                actual_limit = min(val_limit, len(val_dataset))
                # 使用 select 来切分，效率很高，不会实际拷贝数据
                val_dataset = val_dataset.shuffle(seed=42).select(range(actual_limit))
            self.val_loader = DataLoader(val_dataset, shuffle=False, collate_fn=self.collate_fn, batch_size=batch_size, num_workers=num_workers, pin_memory=True)
