...
ROOT = os.getenv('DATASET_ROOT', 'data/')  # Set the root directory to the dataset here
...
def prepare_test_loaders(config):
    ...
    if config.get('val_limit', None) is not None and isinstance(config['val_limit'], int):
        actual_limit = min(config['val_limit'], len(val_dataset))
        
        # 1. 创建一个生成器并设置种子 (对应你之前的 seed=42)
        g = torch.Generator()
        g.manual_seed(42)
        
        # 2. 生成随机索引 (0 到 len-1)
        indices = torch.randperm(len(val_dataset), generator=g).tolist()
        
        # 3. 截取前 actual_limit 个索引
        selected_indices = indices[:actual_limit]
        
        # 4. 使用 Subset 创建一个新的数据集，只包含选中的那些索引
        val_dataset = torch.utils.data.Subset(val_dataset, selected_indices)
